...
<div class="item-status" style="margin-top:-5px;">
    <span class="status-type2"> <?php
      ...
      Je bekijkt de <b>Accountspagina</b></span>
  </div>
  </div>
...
<div class="projects-section">
    <div class="projects-section-header-accounts">
    ...
    <div class="projects-section-line-accounts">

    </div>

  <div class="project-boxes jsGridView">

  <div class="accountspaginatitel">      
<h2>Account aanmaken</h2></div>
  ...
  <tr>
    ...
    <td> <b>Gebruiker</b> </td>
    <td> <input type="text" placeholder="Volledige naam" name="naam" id="naam" value="" class="accountboxinputadd" style="width:140px;" required> </td>
    <td> <input type="text" placeholder="Gebruikersnaam" name="username" id="username" value="" class="accountboxinputadd" style="width:140px;" required> </td>
    <td> <input type="text" placeholder="naam@email.com" name="email" id="email" value="" class="accountboxinputadd" required> </td>

    <td><select id="accounttype" name="accounttype">
    <option value="Student">Student</option>
    <option value="Coach">Coach</option>
    <option value="IVS Managament">IVS Managament</option>
    <option value="0">Gedeactiveerd</option>
    </select></td>

    <td> <input type="password" placeholder="Wachtwoord*" name="password_1" id="password_1" value="" class="accountboxinputadd" style="width:110px;" required> </td>
    <td> <input type="password" placeholder="Herhaal wachtwoord*" name="password_2" id="password_2" value="" class="accountboxinputadd" required> </td>

    <!-- <td> <input type="password" placeholder="Wachtwoord*" name="password_1" id="password_1" value="" class="accountboxinput" required><input type="checkbox" onclick="ShowPW1()"> </td>
    <td> <input type="password" placeholder="Herhaal wachtwoord*" name="password_2" id="password_2" value="" class="accountboxinput" required><input type="checkbox" onclick="ShowPW2()"> </td> -->

    <td> <button type="submit" class="authbutton" name="reg_user">Toevoegen</button> </td>
    </tr>
		</form>
    ...
    <td> <button type="submit" class="authbutton" name="reg_admin">Toevoegen</button> </td>
    </tr>
		</form>
</table>
<p class="informatietekst"><i>Als je een admin / beheerder account aanmaakt krijgt deze gebruiker een normaal account met admin rechten. Je hoeft dus geen extra gebruikersaccount aan te maken.</i></p>
      ...
      if(isset($_POST['update'])) // als er op de update knop wordt gedrukt
        {
          ...
          $sql = "UPDATE users SET naam='$naam', username='$username', email='$email', accounttype='$accounttype' WHERE users.id = $account_id;";

          if ($conn->query($sql) === TRUE) {
          echo "<p class='errortekst'> Bijgewerkte gegevens uploaden naar de database...</p>";
          echo "<meta http-equiv='refresh' content='0'>";
          } else {
          echo "<p class='errortekst'> We kunnen op dit moment geen verbinding maken met de server. Probeer het later nog een keer.</p>"; //<br><br><b>Error:</b><br><br>" . $conn->error;
          }
        ...
        if(isset($_POST['delete'])) // als er op de delete knop wordt gedrukt
        {
          ...
          $sql = "DELETE FROM users WHERE users.id = $account_id;";
    ...
    <th></th>
      ...
      <?php
        ...
        while($row = mysqli_fetch_assoc($rows))
        {
          ...
          echo'<td>' , $row["id"] , '</td>';
          echo'<td> <input type="text" value="' , $row['naam'] , '" name="naam" class="accountboxinput"> </td>';
          echo'<td> <input type="text" value="' , $row['username'] , '" name="username" class="accountboxinput"> </td>';
          echo'<td> <input type="text" value="' , $row['email'] , '" name="email" class="accountboxinput"> </td>';

          echo'<td><select id="accounttype" name="accounttype">';
          echo'<option value="' , $row['accounttype'] , '">' , $row['accounttype'] , '</option>';
          echo'<option value="Student">Student</option>';
          echo'<option value="Coach">Coach</option>';
          echo'<option value="IVS Managament">IVS Managament</option>';
          echo'<option value="0">Gedeactiveerd</option>';
          echo'</select></td>';

          echo'<input type="hidden" value="' , $row['id'] , '" name="account_id">';

          echo'<td> <button type="submit" name="update" value="update" class="projectstatusbutton">Wijzigingen opslaan</button> </td>';
          echo'<td> <button type="submit" name="delete" value="delete" class="projectstatusbuttondelete" onclick="return confirm(\'Weet je zeker dat je het account van ' , $row['naam'] , ' wilt verwijderen? Druk op OK om te verwijderen.\')"; style="margin-left:5px;"> Account verwijderen</button><br></form> </td>';
          echo'</tr>';
        }
      ?>
    ...
    <th></th>
      ...
      <?php
        ...
        while($row = mysqli_fetch_assoc($rows))
        {
          ...
          echo'<td> <button type="submit" name="updateadmin" value="updateadmin" class="projectstatusbutton">Wijzigingen opslaan</button> </td>';
          echo'<td> <button type="submit" name="deleteadmin" value="deleteadmin" class="projectstatusbuttondelete" onclick="return confirm(\'Weet je zeker dat je de admin rechten van ' , $row['naam'] , ' wilt ontnemen? Deze persoon heeft wel nog toegang tot de applicatie maar niet meer tot het admin gedeelte van de applicatie. Druk op OK om te verwijderen.\')"; style="margin-left:5px;"> Admin rechten ontnemen</button><br></form> </td>';
          echo'</tr>';
        }
      ?>
      ...
      if(isset($_POST['updateadmin'])) // als er op de update knop wordt gedrukt
        {
          $account_id = $_POST['account_id'];
          $naam = mysqli_real_escape_string($conn, $_POST['naam']);
          $username = mysqli_real_escape_string($conn, $_POST['username']);
          ...
          $sql = "UPDATE accounts SET naam='$naam', username='$username', email='$email', accounttype='$accounttype' WHERE accounts.id = $account_id;";
        ...
        if(isset($_POST['deleteadmin'])) // als er op de delete knop wordt gedrukt
        {
          $account_id = $_POST['account_id'];
          $naam = mysqli_real_escape_string($conn, $_POST['naam']);
          $username = mysqli_real_escape_string($conn, $_POST['username']);
          ...
          $sql = "DELETE FROM accounts WHERE accounts.id = $account_id;";
