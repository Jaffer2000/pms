...
if ($_POST != NULL) {
    ...
    if ($_POST['action'] == "Toevoegen") {
        ...
        if (strlen($naam)> 50){
            echo '<script>';
            echo 'alert("Product naam mag niet langer dan 50 tekens zijn")';
            echo '</script>';
        } else {
            $product = new UitleenProduct();
            $product->createProduct($naam);
        }
    } elseif ($_POST['action'] == "Bewerken") {
...
<div class="item-status" style="margin-top:-5px;">
    <span class="status-type2"> <?php
      ...
      Je bekijkt het <b>Uitleensysteem producten overzicht</b></span>
  </div>
  </div>
...
<a class="mode-switch" title="Switch Theme">
    ...
    <div class="projects-section">
        <div class="projects-section-header">
            <p>Producten beheer</p>

            <div class="newprojectbutton">
                <a href="uitleen_admin_aanvragen_overzicht.php"><button> Terug naar aanvragen overzicht</button></a>
                <a href="admin.php"><button><i class="fas fa-arrow-circle-left"></i> Terug naar admin portaal</button></a>
            </div>
        </div>

        <form action="uitleen_admin_producten.php" method="post">
            <div>
                <input type="text" class="uitleenproducttextinput" name="naam" placeholder="Voeg hier een product toe" required>
                <input type="submit" class="uitleenproductbeheerbutton" name="action" value="Toevoegen">
            </div>
        </form>
        ...
        <div class="producten">
            ...
            <div class="product">
                <form action="uitleen_admin_producten.php" method="post">
                        ...
                        <input type="text" class="uitleenproducttextinput" name="naam" value="<?php echo htmlspecialchars($product['product_naam']); ?>" required>
                        <input type="submit" class="uitleenproductbeheerbutton" name="action" value="Bewerken">
                        <input type="submit" class="uitleenproductbeheerbutton" name="action" value="Verwijderen" onclick="return confirm('Weet je zeker dat je dit product wil verwijderen?');">
                        <p style="display:inline" class="laatstuitgeleendaanopmaak">Laatst uitgeleend aan:</p>
                </form>
            </div>
            <?php } ?>
        </div>
