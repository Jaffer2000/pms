...
$sql = "SELECT administratienummer FROM project ORDER BY id DESC LIMIT 1;";
$resultadministratienummer = mysqli_query($conn, $sql);
$resultCheckadministratienummer = mysqli_num_rows($resultadministratienummer);
...
<div class="item-status" style="margin-top:-5px;">
    <span class="status-type2"> <?php
      ...
      Je bekijkt het <b>Admin portaal</b></span>
  </div>
  </div>
...
<?php
    ...
    echo'<div class="box-admin-portaal1" style="width:380px; flex-grow: 1;">';
    echo'<div class="project-box1" style="background-color: ', $BGcolor ,';">';
    echo'<h1>Project toevoegen</h1>';
    echo'<p>Een nieuw project toevoegen aan <br>de projectenlijst</p>';

    // Start van de modal (Popup)
    echo'<details><summary>';
    echo'<div class="buttonmodal2">';
    echo'Project toevoegen 🆕</i>';
    echo'</div>';
    ...
    echo'<h1><b><input type="text" placeholder="Projectnaam" name="projectnaam" class="modalboxinputtitle" style="margin-top:10px;" required></b></h1>';
    echo'<p><b>Projectleider: <input type="text" placeholder="Nog geen projectleider" name="projectleider" class="modalboxinput" required></b></p>';

    echo'<div class="details-modal-bewerkt-door" style="margin-top: -75px; text-align:left;">';
    echo'<p><b>Uitleg projectnummer:</b><br><br>';
    echo'<b>1e letter:</b> E = Extern, I = Intern project.<br>';
    echo'<b>2e letter:</b> S = Software, M = Media, C = Combinatie project.<br>';
    echo'<b>3e:</b> Jaartal en Jaarnummer project.<br>';
    echo'<b><i>Projectnummer van het nieuwste project staat ingevuld!</i></b></p>';
    echo'</div>';

    // Select voor de Prioriteit
    echo'<label for="prio"><b>Prioriteit: </b></label>';
    echo'<select id="prio" name="prio" required>';
    echo'<option value="Normaal">Kies Prioriteit</option>';
    ...
    echo'<select id="project_status" name="project_status" required>';
    echo'<option value="Initiatie">Selecteer Status</option>';
    ...
    echo'<select id="opleiding" name="opleiding" required>';
    echo'<option value="Software">Kies een opleiding</option>';
    ...
    echo'<p><b>Klant: </b><input type="text" placeholder="Voornaam Achternaam" name="klant" class="modalboxinput" required>';
    echo'<p><b>Projectcoach: </b><input type="text" placeholder="Voornaam Achternaam" name="projectcoach" class="modalboxinput" required>';
    echo'<p><b>Telefoonnummer Klant: </b><input type="text" placeholder="+31 6 000 000 00" name="telefoonnummer" class="modalboxinput" required>';
    echo'<p><b>Email Klant: </b><input type="text" placeholder="naam@email.com" name="emailklant" class="modalboxinput" required>';
    echo'<p><b>Projectleden: </b><input type="text" placeholder="Voornaam Achternaam" name="projectleden" class="modalboxinput" required>';
    echo'<p><b>Initiatieleider: </b><input type="text" placeholder="Voornaam Achternaam" name="initiatieleider" class="modalboxinput" required>';
  
  if ($resultCheckadministratienummer > 0) {
    while($row = mysqli_fetch_assoc($resultadministratienummer)) {
      echo'<p><b>Projectnummer: </b><input type="text" value="' , $row['administratienummer'] , '" name="administratienummer" class="modalboxinput" maxlength="10" placeholder="EC-2022-00" required>';
    }
  }
    ...
    echo'<textarea style="white-space:pre-wrap; width:100%;" class="modalboxinputtxtarea" maxlength="90000" placeholder="Vul hier de gegevens in die de klant meegeeft bij de projectaanvraag. Vergeet na het toevoegen van het project aan de PMS niet een bevestigingsmail te sturen naar de klant." name="opmerkingen" required>' , $row['opmerkingen'] , '</textarea>';
    echo'<button type="submit" name="submit" value="submit" class="projectstatusbutton" style="margin-left:5px; float:right;">Project toevoegen</button>';
    echo'</form>';
    echo'</div>';
...
<article class="box-admin-portaal">
  ...
  <button onclick="window.location='uitleen_admin_aanvragen_overzicht.php';" class="breakout-button" type="button">Producten beheren ➕</button>
</article>
...
<article class="box-admin-portaal">
  ...
  if(isset($_POST['submit'])) // als er op de submit knop wordt gedrukt
    {
    ...
    $sql = "INSERT INTO project (projectnaam , projectleider , klant , prio , projectcoach , telefoonnummer , emailklant , projectleden , initiatieleider , project_status , opmerkingen, opleiding, administratienummer, bewerkt_door, bewerkt_datum)
      VALUES ('$projectnaam', '$projectleider' , '$klant' , '$prio' , '$projectcoach' , '$telefoonnummer' , '$emailklant' , '$projectleden' , '$initiatieleider' , '$project_status' , '$opmerkingen' , '$opleiding' , '$administratienummer' , '$bewerkt_door' , '$bewerkt_datum')";

    if (mysqli_query($conn , $sql)) {
      echo'<script src="https://cdnjs.cloudflare.com/ajax/libs/prefixfree/1.0.7/prefixfree.min.js"></script>
      <div class="alert2 error" style="z-index: 1; margin-top: -20px;">
      <input type="checkbox" id="alert1" />
      <label class="close" title="close" for="alert1">
        <i class="icon-remove"></i>
      </label>
      <p class="inner">
        Bedankt voor je toevoeging ' , ucfirst($_SESSION['name']) , '! Het project is succesvol toegevoegd aan het IVS Project management systeem! 
      </p>
      </div>';
    } else {
