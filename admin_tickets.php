...
$sql = "SELECT * FROM `tickets` WHERE project_status='Bezig';";

$bezig = mysqli_query($conn, $sql);
$resultBezig = mysqli_num_rows($bezig);


$sql = "SELECT * FROM `tickets` WHERE project_status='Ingediend';";

$InIngediend = mysqli_query($conn, $sql);
$resultIngediend = mysqli_num_rows($InIngediend);


$sql = "SELECT * FROM `tickets` WHERE project_status='Afgerond';";
...
$sql = "SELECT * FROM `tickets` WHERE `projectnaam` LIKE '%".$searchresult."%' OR `beheerder` LIKE '%".$searchresult."%' OR `klant` LIKE '%".$searchresult."%';";
...
<div class="item-status" style="margin-top:-5px;">
    <span class="status-type2"> <?php
      ...
      Je bekijkt de <b>Admin Ticketpagina</b></span>
  </div>
  <div class="search-wrapper">
        <form action="" method="GET">
        <input class="search-input" type="text" placeholder="Zoek in tickets..." name="search" id="search" value="">
...
<div class="app-content">
  ...
  <div class="projects-section">
    <div class="projects-section-header">
      ...
      <a href="admin.php"><button><i class="fas fa-arrow-circle-left"></i> Admin portaal</button></a>
    </div>
    </div>
    <div class="projects-section-line">
      ...
      <form action="" method="post" class="statusformulier">
        <div class="status-type"><span class="bezig" style="background: #E66600"><?php echo $resultIngediend; ?></span><input type="submit" class="filterbutton" name="ingediend" value="Ingediend"><i class="fa-solid fa-arrow-right" style="color:white; margin-left:6px;"></i></div>
        <div class="status-type"><span class="bezig" style="background: #27C840"><?php echo $resultBezig; ?></span><input type="submit" class="filterbutton" name="bezig" value="Bezig"><i class="fa-solid fa-arrow-right" style="color:white; margin-left:6px;"></i></div>
        ...
        <div class="status-type" style="margin-left:10px;"><span class="bezig" style="background: #FFFFFF"><?php echo $resultCheck; ?></span><a href="tickets.php" class="filterbutton"><b style="margin-right:5px;"> Totaal actief</b></a></div>
      </div>
  ...
  <div class="project-boxes jsGridView">
      <?php
        ...
        } elseif (isset($_POST["ingediend"])) {    
          $result = $InIngediend;
          $resultCheck = $resultIngediend;
        ...
        if ($resultCheck > 0) {
          while($row = mysqli_fetch_assoc($result)) {
            ...
            echo'Ticket bewerken <i class="fas fa-chevron-circle-right"></i>';
            echo'</div>';
            ...
            echo'<h1><b><input type="text" value="' , $row['projectnaam'] , '" name="projectnaam" class="modalboxinput"></b></h1>';
            echo'<p><b>Ticketbeheerder: <input type="text" value="' , $row['beheerder'] , '" name="beheerder" class="modalboxinput"></b></p>';
            ...
            echo'<label for="project_status">Ticket Status: </label>';
            ...
            echo'<option value="Ingediend">Ingediend</option>';
            ...
            echo'<p><b>Email Klant: </b><input type="text" value="' , $row['email'] , '" name="email" class="modalboxinput">';
            echo'<p><b>Korte omschrijving: </b><input type="text" maxlength="50" value="' , $row['korte'] , '" name="korte" class="modalboxinput">';
            echo'<p><b>Projectcoach: </b><input type="text" value="' , $row['projectcoach'] , '" name="projectcoach" class="modalboxinput">';
            echo'<p><b>Indiener: </b><input type="text" value="' , $row['indiener'] , '" name="indiener" class="modalboxinput">';
            echo'<p><b>Ticketnummer: </b><input type="text" value="' , $row['ticketnummer'] , '" name="ticketnummer" class="modalboxinput" maxlength="10" placeholder="EC-2022-00">';
            echo'<input type="hidden" value="' , $row['id'] , '" name="ticket_id">';

            echo'</div>';

            echo'<div class="modalbox">';

            echo'<p style="margin-bottom: 5px;"><b>Ticketomschrijving: </b></p>';
            echo'<textarea style="white-space:pre-wrap; width:100%;" class="modalboxinputtxtarea" maxlength="90000" value=' , $row['ticketomschrijving'] , ' name="ticketomschrijving">' , $row['ticketomschrijving'] , '</textarea>';
            ...
            echo'<button type="submit" name="delete" value="delete" class="projectstatusbuttondelete" onclick="return confirm(\'Weet je zeker dat je dit ticket wilt verwijderen? Druk op OK om te verwijderen.\')"; style="float:right;">Verwijderen</button><br></form>';
            echo'</div>';
          ...
          echo'<div class="project-box-wrapper">';
          echo'<h3>Geen tickets gevonden</h3>';
          echo'</div>';
        }
      ?>
      <?php
        ...
        if(isset($_POST['update'])) // als er op de update knop wordt gedrukt
          {
            ...
            $beheerder = mysqli_real_escape_string($conn, $_POST['beheerder']);
            $projectcoach = mysqli_real_escape_string($conn, $_POST['projectcoach']);
            $email = mysqli_real_escape_string($conn, $_POST['email']);
            $korte = mysqli_real_escape_string($conn, $_POST['korte']);
            $indiener = mysqli_real_escape_string($conn, $_POST['indiener']);
            $ticketnummer = mysqli_real_escape_string($conn, $_POST['ticketnummer']);
            $project_status = mysqli_real_escape_string($conn, $_POST['project_status']);
            $prio = mysqli_real_escape_string($conn, $_POST['prio']);
            $ticketomschrijving = mysqli_real_escape_string($conn, $_POST['ticketomschrijving']);

        $sql = "UPDATE tickets SET klant='$klant', projectcoach='$projectcoach', email='$email', korte='$korte', indiener='$indiener', beheerder='$beheerder', projectnaam='$projectnaam', ticketomschrijving='$ticketomschrijving', ticketnummer='$ticketnummer', project_status='$project_status', prio='$prio' WHERE tickets.id = $ticket_id;";

        if ($conn->query($sql) === TRUE) {
        echo "Updates doorvoeren...";
        ...
        $sql = "DELETE FROM tickets WHERE tickets.id = $ticket_id;";
    
        if ($conn->query($sql) === TRUE) {
        echo "Ticket verwijderen...";
        echo "<meta http-equiv='refresh' content='0'>";
        } else {
        echo "Sorry, het lijkt erop dat er iets fout is gegaan bij het verwijderen van dit ticket. Probeer het later nog een keer."; //<br><b>Error:</b><br>" . $conn->error;
        }
